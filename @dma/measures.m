...
function [m] = measures(D, window)
  ...
  m.g     = 0; %generalised anti-robinson violation          (loss)  %Wu, Tien & Chen 2010
  m.rg    = 0; %relative generalised anti-robinson violation (loss)  %Wu, Tien & Chen 2010
  ...
  if n ~= d %for two-mode matrices, ignore all measures below
    return;
  end
  ...
  for i = 2 : n-1
    ...
    tr  = tril(ones(n-i),window-i);
    m.g = m.g + sum(sum( sign_hor.*tr + sign_ver.*tr ));
  end

  m.rg = m.g / ( 2 * ( n * window*(window-1)/2 - (window^3-window)/3 ) );
