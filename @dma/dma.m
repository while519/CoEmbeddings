...
%Includes various algorithms, utilities and measures for
%one-mode and two-mode seriation, and processing of permutation information
%
%ver 1.2
%JY Goulermas, 2010
...
classdef dma
  ...
  properties
    ...
    error_window  = 15;    %for one error in dma.measures()
    ...
    tsp_args      = {};    %'starts' to set the number of path start attempts in the solver
    qap_args      = {};    %
    spectral_args = {};    %'method' is the name of the method, 'ext_perm' an external permutation vector, 'ext_mix' its mixture coefficient
    ga_args       = {};    %'objective' is one of: 'tsp', 'qap', era, arc', etc, W is the template for the 'qap' case, 
    dmls_args     = {};    %
  end
  ...
  methods
    ...
    function obj = analyse(obj)
      obj.scores.D = dma.measures(obj.D, obj.error_window);
      obj.scores.Q = dma.measures(obj.Q, obj.error_window);
    end
