function [ Xq, Yq, time, para, best, exitflag, svds, Rqz, Rq ] = LEDFsvd( R, dim, options, para_range )
%%
...
if ~exist('zeta_lower','var')
    switch lower(opt)
        case 'ga'
            clc
            S=sprintf(['It is current running LEDFsvd' '.....diff_alpha']);
            disp(S);
            ...
            time = cputime;
            [x,fval,exitflag] = ga(@(x) LEDFsvd_obj(x, R, dim, obj_type, obj_para),dim+3,[],[],[],[],...
                [eta1_lower, eta2_lower, repmat(alpha_lower,1,dim), beta_lower],...
                [eta1_upper, eta2_upper, repmat(alpha_upper,1,dim), beta_upper],...
                [],[],options1);
            para.eta1 = x(1);
            para.eta2 = x(2);
    ...
    switch lower(opt)
        case 'ga'
            clc
            S=sprintf(['It is current running LEDFsvdz' '.....diff_zeta']);
            disp(S);
            options1 = gaoptimset('PopulationSize',104,'UseParallel', false,...
                'Vectorized', 'on','Display','iter',...
                'PlotFcns',@gaplotbestf...
            ...
            time = cputime;
            [x,fval,exitflag] = ga(@(x) LEDFsvdz_obj(x, R, dim, obj_type, obj_para),dim+3,[],[],[],[],...
                [eta1_lower, eta2_lower, repmat(zeta_lower,1,dim), beta_lower],...
                [eta1_upper, eta2_upper, repmat(zeta_upper,1,dim), beta_upper],...
                [],[],options1);
            para.eta1 = x(1);
            para.eta2 = x(2);
            para.zeta = x(3:dim+2);
