%Use a genetic algorithm to optimise various seration scores.
%
%D     : an nxn distance matrix.
%score : the type of GA fitness score used for seriation
...
function [perm, details] = ga(D, varargin)
  ...
  fitness  = @(x) ObjectiveFunction(x, objective, D, W);
  creation = @(x,y,z) CreatePopulation(x, y, z, D);
  options  = gaoptimset('PopulationType',    'custom', ...
                        ...
                        'EliteCount',        2, ...
                        'SelectionFcn',      @selectionroulette, ... or selectionstochunif
  ...
  [opt, details.fval, details.reason, details.output] = ga(fitness, n, options);
  perm = opt{:}';
...
function [pop] = CreatePopulation(n, ~, options, D)
  ...
  if false %hybridisation with some resonably good solutions%TO BE REMOVED ELSEWHERE!!!
    o         = dma(D);
    o.hc_args = {'method', 'average', 'ordering', 'olo'};
    o.algo    = 'vat';  pop{1} = o.run().perm';
    o.algo    = 'hc';   pop{2} = o.run().perm';
    o.algo    = 'r2e';  pop{3} = o.run().perm';
  end
...
function [p] = CrossoverOrder1(p1, p2)
  ...
  [~,not_idx]             = setdiff(p2, p(start:stop));
