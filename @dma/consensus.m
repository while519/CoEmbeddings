...
function [perm, details] = consensus(P, w, varargin)
  ...
  ppm_algo    = 'spectral';
  ppm_ga_args = {'generations', 500, 'pop_size', 80, 'tol_fun', 1e-7};
  ...
  switch lower(method)
    ...
    case 'ppm'
      K = PositionalDistanceMatrix(P, w);
    ...
    otherwise
      error('dma:consensus', 'unknown ppm algo: %s', ppm_algo )
  end
...
function [K] = PositionalDistanceMatrix(P, w)
  [n,b] = size(P);
  K     = zeros(n);
  for k = 1 : b
    p = dma.invperm(P(:,k));
...
function [p] = CMedian(P,w)
  ...
  options = optimset('BranchStrategy', 'maxinfeas', ...
                     'MaxIter',        1E6*nv, ...
                     'Display',        'off');
  opt     = bintprog(-c, A, b, [], [], [], options );
