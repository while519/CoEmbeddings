function [ Xq, Yq, time, para, best, exitflag, svds, Rqz, Rq ] = LEDsvd( R, dim, options, para_range )
%%
...
if ~exist('zeta_lower','var')
    switch lower(opt)
        case 'ga'
            clc
            S=sprintf(['It is current running LEDsvd' '.....eq_alpha']);
            disp(S);
            if ~isequal(eta1_upper, eta1_lower) || ~isequal(eta2_upper,eta2_lower) || ...
                ...
                time = cputime;
                [x,fval,exitflag] = ga(@(x) LEDsvd_obj(x, R, dim, obj_type, obj_para),4,[],[],[],[],...
    ...
    switch lower(opt)
        case 'ga'
            clc
            S=sprintf(['It is current running LEDsvdz' '.....eq_zeta']);
            disp(S);
            if ~isequal(eta1_upper, eta1_lower) || ~isequal(eta2_upper,eta2_lower) || ...
                   ~isequal(zeta_lower,zeta_upper) || ~isequal(beta_lower, beta_upper)
                options1 = gaoptimset('PopulationSize',52,'UseParallel', false,...
                    'Vectorized', 'on','Display','iter',...
                    'PlotFcns',@gaplotbestf...
                    );
                time = cputime;
                [x,fval,exitflag] = ga(@(x) LEDsvdz_obj(x, R, dim, obj_type, obj_para),4,[],[],[],[],...
                    [eta1_lower, eta2_lower, zeta_lower, beta_lower],...
                    [eta1_upper, eta2_upper, zeta_upper, beta_upper],...
                    [],[1,2],options1);
                time = cputime-time;
                best = fval;
            else
                x = [eta1_lower, eta2_lower, zeta_lower, beta_lower];
                time = 0;
                best = 0;
                exitflag = -100;
            end
